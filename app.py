...
st.set_page_config(page_title='🛡️ كتيبة الفرسان', layout='wide')

# إلغاء الفراغات نهائياً لتوسيع شاشة اللعب على الهواتف
...
st.title('🛡️ كتيبة الفرسان: حرب الشوارع والصمود')
st.write('🎮 نمط رماية تكتيكي: تحرك، اقفز فوق السيارات والأنقاض، وتصدى مع فرقتك ذات العُصب الخضراء لجنود الميليشيات الغازية!')

# كود لعبة الرماية والمواجهة من المنظور الجانبي الحماسي والأمن تماماً
...
h += '<div id="mainMenu" class="menu-screen"><h1 style="color:#10b981;margin:0 0 10px 0;font-size:28px;text-shadow:0 0 15px #10b981;">🛡️ كتيبة الفرسان: معركة الصمود</h1><p style="color:#9ca3af;margin:0 0 25px 0;font-size:14px;line-height:1.5;">تحرك في الشارع، اقفز فوق السيارات لحماية نفسك، وتصدّى مع أفراد فرقتك ذوي العُصب الخضراء لجنود الميليشيات الغازية!</p><button class="btn" onclick="startGame()">اقتحام الميدان والاشتباك ⚡</button></div>'
h += '<div id="gameOverMenu" class="menu-screen" style="display:none;"><h1 style="color:#ef4444;margin:0 0 10px 0;">💥 انتهت المواجهة</h1><p id="finalScore" style="font-size:18px;color:#d1d5db;margin:0 0 25px 0;"></p><button class="btn" style="background:linear-gradient(135deg,#10b981,#1f2937);" onclick="startGame()">إعادة تنظيم الصفوف</button></div>'
h += '</div></div>'

h += '<div class="controls"><div class="ctrl-btn" id="btnLeft">◀ تراجع</div><div class="ctrl-btn" id="btnRight">تقدم ▶</div><div class="ctrl-btn" id="btnJump">🦘 قفز</div><div class="ctrl-btn" id="btnFire">🔥 FIRE</div></div>'
...
h += 'function spawnEnemy(){if(!gameActive)return;if(enemies.length<3){enemies.push({x:canvas.width+20,y:groundY-40,w:24,h:40,s:1.6+Math.random(),lastFire:Date.now()});}setTimeout(spawnEnemy,2200);}'
...
h += 'if(moveLeft){player.x-= player.speed || 4;player.facing=-1;}if(moveRight){player.x+= player.speed || 4;player.facing=1;}if(player.x<0)player.x=0;if(player.x>canvas.width-player.w)player.x=canvas.width-player.w;'
...
h += 'drawSoldier(player.x-35,player.y+4,player.w-2,player.h-2,true,false,player.facing);'

h += 'let now=Date.now();if(isFiring&&now-lastPlayerFire>250){bullets.push({x:player.facing===1?player.x+player.w:player.x,y:player.y+player.h/2,vx:player.facing*8,vy:0});bullets.push({x:player.facing===1?player.x:player.x-35,y:player.y+player.h/2,vx:player.facing*8,vy:0});lastPlayerFire=now;}'
...
h += 'if(now-e.lastFire>1600){enemyBullets.push({x:e.x,y:e.y+e.h/2,vx:-5,vy:0});e.lastFire=now;}'
...
h += 'requestAnimationFrame(animate);}function go(){gameActive=false;document.getElementById("gameOverMenu").style.display="flex";document.getElementById("finalScore").innerText="عدد الأعداء الذين تم رصدهم: "+score;}</script></body></html>'
